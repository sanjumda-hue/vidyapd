-- =============================================================================
-- 020_prediction_quota_eligibility.sql
--
-- Teach the predictor the two quota rules added in 019: a quota tied to one
-- state, and a quota that needs a separately-claimed eligibility.
--
-- Forward-only migrations, so this restates the whole function.
-- =============================================================================

BEGIN;

DROP FUNCTION IF EXISTS fn_predict_colleges(SMALLINT, BIGINT, SMALLINT, applicant_gender,
  BOOLEAN, SMALLINT, SMALLINT[], SMALLINT[], college_type[], TEXT, INT);
...
CREATE OR REPLACE FUNCTION fn_predict_colleges(
  ...
  /** Include quotas that need a separately-claimed eligibility (DASA, AF, FF). */
...
BEGIN
  ...
  candidate AS (
    ...
    WHERE t.exam_id = p_exam_id
      ...
      -- Home-state / other-state eligibility, relative to the college.
      ...
      -- Quotas tied to one particular state, wherever the college is. NIT
      -- Goa's Goa Quota is for Goa residents, not for whoever applies to it.
      AND (q.home_state_id IS NULL OR q.home_state_id = p_home_state_id)
      -- DASA, Armed Forces and similar need a separate eligibility the
      -- candidate has to claim. Offering them by default would put seats in
      -- front of students who cannot take them.
